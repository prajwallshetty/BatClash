...
export default function EditProblemPage() {
  ...
  const fetchProblem = async () => {
    try {
      ...
      setVisibleTestCases(
        problem.visibleTestCases.map((tc: any) => ({
      ...
      setHiddenTestCases(
        problem.hiddenTestCases.map((tc: any) => ({
          input: JSON.stringify(tc.input, null, 2),
          expectedOutput: JSON.stringify(tc.expectedOutput, null, 2),
          description: tc.description || '',
        }))
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-foreground/70">Loading problem...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-foreground/70">Update problem details</p>
      </div>
